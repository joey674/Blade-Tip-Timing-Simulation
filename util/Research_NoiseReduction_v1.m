function Research_NoiseReduction_v1(blade, EO)
    ...
    for blade_idx = 1:n_blades
        ...
        for i = 1:length(density_levels)
            ...
            %
            indices = [ find(downsampled_magn > err_value_lower_current & downsampled_magn < err_value_lower_last),...
            ...
            num_to_pick = round(downsample_factors(i)); % 每组中需要选取的元素数量
            ...
            for start_idx = 1:group_size:num_elements
                ...
                current_group = indices(start_idx:end_idx); 
        ...
        %%
        downsampled_magn = smoothdata(downsampled_magn, 'movmean', 200);
        
        %% 插值以恢复数据长度
        interpolated_magn = interp1(downsampled_freq, downsampled_magn, freq, 'linear', 'extrap');
        
        figure('units', 'normalized', 'outerposition', [0 0 1 1]); set(gcf, 'WindowStyle', 'docked'); title('method comparison');
        plot(freq, smoothdata(magn, 'movmean', 200), 'LineWidth', 2, 'DisplayName', 'Movmean'); hold on;
        plot(freq, interpolated_magn, 'LineWidth', 1, 'DisplayName', 'Movmean after downsample'); hold on;
        legend;
        hold off;
    end
end
